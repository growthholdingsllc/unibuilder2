   ...
   var table_name = typeof dbobject.this_table !== 'undefined' ? dbobject.this_table.table_name : 'notable';
	...
	var table =  dbobject.tableName.dataTable({
			...
			"fnRowCallback": function(nRow, data, iDisplayIndex, iDisplayIndexFull, status) {
				...
				if( table_name == 'Builder_Users')
				{
					if ( data[dbobject.role_id] == 1)
					{
						$('td:eq('+dbobject.delete_data.index+')', nRow).html('<input type="checkbox" value="'+ 0 +'" class="chk"  />');
					}
				}
				...
				if( table_name == 'Projects_List')
				{
					...
					if(typeof(data['contract_price']) != "undefined")
					{
						// alert(data['contract_price']);
						$('td:eq('+dbobject.contract_price_data.index+')', nRow).html(parseFloat(data['contract_price']).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, "$1,"));	
					}
							
				}
				if( table_name == 'sub_contractor_list')
				{
					 //alert(data[dbobject.division].length);
					if(data[dbobject.length] > 25)
					{
						$('td:eq('+dbobject.division_index.index+')', nRow).append('<strong> ......</strong>');
					}
				}
				...
				if( table_name == 'Selection_List')
				{
						...
						/* $('td:eq('+dbobject.allowance.index+')', nRow).html('');
						$('td:eq('+dbobject.allowance.index+')', nRow).append('$'+data[dbobject.allowance_val]); */
				...
				{
						if(data[dbobject.total_paid_amount] == 0.00)
						{
				...
				if( table_name == 'budget_jobs_list')
				{
					...
					if(budget_jobs_client_co_count_index != '')
					{
					...
					if(budget_jobs_client_contract_count_index != '')
					{
				...
				if( table_name == 'budget_jobs_list')
				{
					//alert('po'+data['po_count']);
					//alert('po re'+data['po_release_count']);
					if(typeof(data['po_release_count']) != "undefined")
					{
						//alert(data['plus_minus_budget'].indexOf('-'));
						if ( data['po_release_count'] == data['po_count'])
						{
							$('td:eq('+dbobject.job_data.index+')', nRow).html('<a class="edit-role activity_tab_act" href="#" data-toggle="modal" data-target="#costcode" onclick="add_jobs(0,'+data[dbobject.job_id]+');">'+data[dbobject.job_name]+'</a>');

							//$('td:eq('+dbobject.job_data.index+')', nRow).html('<div class="text-success">'+data['po_release_count']+'</div>');

						}else{
							$('td:eq('+dbobject.job_data.index+')', nRow).html('<a class="edit-role activity_tab_act text-danger" href="#" data-toggle="modal" data-target="#costcode" onclick="add_jobs(0,'+data[dbobject.job_id]+');">'+data[dbobject.job_name]+'</a>');
							//$('td:eq('+dbobject.job_data.index+')', nRow).html('<div class="text-danger">' +data['po_release_count']+'</div>');
						}
					}

					if(typeof(data['estimated_profit_amount']) != "undefined")
					{
						if ( data['estimated_profit_amount'].indexOf('-') != -1)
						{
					...
					if(typeof(data['plus_minus_budget']) != "undefined")
					{
						//alert(data['plus_minus_budget'].indexOf('-'));
						if ( data['plus_minus_budget'].indexOf('-') != -1)
						{
					...
					if(typeof(data['profit_to_date']) != "undefined")
					{
						if ( data['profit_to_date'].indexOf('-') != -1)
						{
					...
					if(typeof(data['overall_profit']) != "undefined")
					{
						if ( data['overall_profit'].indexOf('-') != -1)
						{
				...
				{
					if(typeof(data['total_plus_minus_budget']) != "undefined")
					{
						if ( data['total_plus_minus_budget'].indexOf('-') != -1)
						{
					...
					if(typeof(data['total_estimated_profit_amount']) != "undefined")
					{
						if ( data['total_estimated_profit_amount'].indexOf('-') != -1)
						{
					...
					if(typeof(data['total_profit_to_date']) != "undefined")
					{
						if ( data['total_profit_to_date'].indexOf('-') != -1)
						{
					...
					if(typeof(data['total_overall_profit']) != "undefined")
					{
						if ( data['total_overall_profit'].indexOf('-') != -1)
						{
					...
					if(typeof(data['total_contract_price']) != "undefined")
					{
					...
					if(typeof(data['total_revised_contract']) != "undefined")
					{
				...
				if( table_name == 'budget_project_list')
				{
					...
					if(typeof(data['total_estimated_profit_amount']) != "undefined")
					{	
						// alert(data['total_estimated_profit_amount'].indexOf('-'));
						if ( data['total_estimated_profit_amount'].indexOf('-') != -1)
						{
							$('td:eq('+dbobject.total_estimated_profit_amount.index+')', nRow).html('<div class="text-danger">'+data['total_estimated_profit_amount']+'</div>');
						}
				...
				if( table_name == 'budget_co_list')
				{
					//$('td:eq('+dbobject.edit_data.index+')', nRow).html('<a class="edit-role activity_tab_act" href="#" data-toggle="modal" data-target="#create_payment" onclick="open_payment_modal('+data[dbobject.id]+');">'+data[dbobject.name]+'</a>');
					//alert(data['paid_amount']);
					if(data['paid_amount'] == null)
					{
						$('td:eq('+dbobject.paid_amount_index.index+')', nRow).html('');
					}
					if(data['total_amount'] == null)
					{
						$('td:eq('+dbobject.cost_index.index+')', nRow).html('');
					}
				}
				if( table_name == 'budget_po_list')
				{
					...
					var encoded_string_edit_role = Base64.encode( dbobject.po_data.url + data[dbobject.id]+'/'+0+'/'+0+'/'+data[dbobject.project_id]);
					var encoded_edit_val = encoded_string_edit_role.strtr(encode_chars_obj);

					//alert(dbobject.po_data.url);
					$('td:eq('+dbobject.po_data.index+')', nRow).html('<a class="edit-role" href="'+base_url + encoded_edit_val + '">Create A CO</a>');
					}
					else
					{
				...
				if( table_name == 'budget_owner_po_list')
				{
					if((data[dbobject.status] == 'Accepted by Builder' || data[dbobject.status] == 'Accepted by Client' || data[dbobject.status] == 'Work Completed')){
					//$('td:eq('+dbobject.po_data.index+')', nRow).html('<a class="edit-role" href="javascript:void(0);" onclick="add_owner_co('+data[dbobject.id]+');">Create A CLIENT CO</a>');
					var encoded_string_edit_role = Base64.encode( dbobject.po_data.url + data[dbobject.id]+'/'+0+'/'+0+'/'+data[dbobject.project_id]);
					var encoded_edit_val = encoded_string_edit_role.strtr(encode_chars_obj);

					$('td:eq('+dbobject.po_data.index+')', nRow).html('<a class="edit-role" href="'+base_url + encoded_edit_val + '">Create A CLIENT CO</a>');
					}
					else
					{
						$('td:eq('+dbobject.po_data.index+')', nRow).html('---');
				...
				{
					var encoded_string_edit_role = Base64.encode( dbobject.view_data.url + data[dbobject.id]);
					var encoded_edit_val = encoded_string_edit_role.strtr(encode_chars_obj);
                    //$('td:eq('+dbobject.view_data.index+')', nRow).html('<a class="edit-role activity_tab_act" href="'+base_url + encoded_edit_val + '">'+data[dbobject.name]+'</a>');
					// alert(data['status']);
					if(data['status'] != 'New')
					{
						$('td:eq('+dbobject.view_data.index+')', nRow).html('<a class="edit-role" href="'+base_url + encoded_edit_val + '">VIEW</a>');
					}
					else
					{
						$('td:eq('+dbobject.view_data.index+')', nRow).html('--');
					}
			...
			if( table_name == 'Bids_List')
				{
						...
						if(data[dbobject.released_date_val] == null)
						{
							//var img_src = base_url + 'assets/images/strip.gif';
							$('td:eq('+dbobject.released_date.index+')', nRow).html('---');
						}
	...
	var table =  dbobject.tableName.dataTable({
		...
		"fnRowCallback": function(nRow, data, iDisplayIndex, iDisplayIndexFull, status) 
		{
			...
			if( table_name == 'Docs_List')
			{
				...
				var doc_folder_contain = '<div class="text-center"><img src="' + imgLink +'new_doc_small.png"/>('+ data.Contains[0].doc_dir_con +')&nbsp;&nbsp;<img src="' + imgLink +'document.png"/> ('+ data.Contains[0].doc_fol_con +')</div>';
				var project_doc_folder_contain = '<div class="text-center"><img src="' + imgLink +'project_doc_small.png"/> ('+ data.Contains[0].doc_dir_con +')&nbsp;&nbsp;<img src="' + imgLink +'document.png"/> ('+ data.Contains[0].doc_fol_con +')</div>';
				var doc_folder_contain_sub_owner = '<div class="text-center"><img src="' + imgLink +'document.png"/> ('+ data.Contains[0].doc_fol_con +')</div>';
				...
				var last_updated = '<div class="text-center">'+ data['LastUpdated'] +'</div>';
				if (account_type == 100)
				{
					...
					$('td:eq(2)', nRow).html(doc_folder_contain_none);                                
					$('td:eq(3)', nRow).html(actions_download);
					$('td:eq(4)', nRow).html(last_updated);
			...
			else
			{
				...
				else
				{
					...
					if(data['file_type'] === "folder")
					{
						...
						var action = '<div class="text-center view_access"><a href="javascript:void(0);" class="edit_album_modal" data-target="#Edit_Album_Modal" data-toggle="modal"><img class="uni_edit" src="' + imgLink +'strip.gif"/></a>'+'&nbsp;&nbsp;'+'<a href="javascript:void(0);" onclick="album_delete(' + data[dbobject.folder_id] + ')"><img class="uni_delete" src="' + imgLink +'strip.gif"/></a><input type="hidden" class="folder_id" value="' + data[dbobject.folder_id] + '"></div>';
						var action_no_project = '<div class="text-center view_access"><a href="javascript:void(0);" onclick="album_delete(' + data[dbobject.folder_id] + ')"><img class="uni_delete" src="' + imgLink +'strip.gif"/></a><input type="hidden" class="folder_id" value="' + data[dbobject.folder_id] + '"></div>';
						var photos = '<div class="text-center">'+ data['Photos'] +'</div>';
						var last_updated = '<div class="text-center">'+ data['LastUpdated'] +'</div>';
						var action_sub_owner = '<div class="text-center">-</div>';
						if (account_type == 100)
						{
					...
					else
					{
						...
						var action_file = '<div class="text-center"><a href="javascript:void(0);" class="edit_album_modal" data-target="#Edit_Album_Modal" data-toggle="modal"><img class="uni_edit" src="' + imgLink +'strip.gif"/></a>'+'&nbsp;&nbsp;&nbsp;'+'<a href="'+ base_url + encoded_download_url +'"><img class="uni_download" src="' + imgLink +'strip.gif"/></a>'+'&nbsp;&nbsp;&nbsp;'+'<a href="javascript:void(0);" onclick="delete_file(' + data[dbobject.folder_id] + ')"><img class="uni_delete" src="' + imgLink +'strip.gif"/></a><input type="hidden" class="file_id" value="' + data[dbobject.folder_id] + '"></div>';
						
						var action_file_sub_owner = '<div class="text-center"><a href="'+ base_url + encoded_download_url +'"><img class="uni_download" src="' + imgLink +'strip.gif"/></a></div>';
						var photos = '<div class="text-center">'+ data['Photos'] +'</div>';
						var last_updated = '<div class="text-center">'+ data['LastUpdated'] +'</div>';
						if (account_type == 100)
						{
...
function ajax_grid_view_action(actiontype)
{
	...
	if($('#grid_settings_columns').val() == '' || $('#list_view_name').val() == '')
	{
		...
		$('.alert_modal_txt').text('Please select a column / view name'); */
		grid_error_box();	
		//grid_success_box();	
		$('.grid_settings .error-message .alerts').text('Please select a column / view name');	
		return false;
	}
	if(actiontype == 'save')
	{
		if($('#current_view_grid_settings_id').val()>0)
		{
			...
			grid_error_box();
			$('.grid_settings .error-message .alerts').text('Grid view is already in the database!');	
			// $('#alertModal').modal('show');
			// $('.alert_modal_txt').text('Grid view is already in the database!');
			return false;
		}
	}
	if(actiontype == 'update')
	{
		...
		else
		{
			...
			grid_error_box();
			$('.grid_settings .error-message .alerts').text('There are no previous saved views! Please try to save it newly');	
			// $('#alertModal').modal('show');
			// $('.alert_modal_txt').text('There are no previous saved views! Please try to save it newly');
			return false;
		}
	}
	switch(actiontype)
	{
			...
			$.ajax({
				...
				success: function(response) {	
					if(response == '')
					{
						...
						grid_error_box();
						$('.grid_settings .error-message .alerts').text('Update was not successful!');
						// $('#alertModal').modal('show');
						// $('.alert_modal_txt').text('Update was not successful!');
					}
					else
					{
						...
						checkbox();
						grid_success_box();	
						$('.grid_settings .error-message .alerts').text('Saved successfully!');	
					}
				}
			});	
			break;
			...
			$.ajax({
				...
				success: function(response) {		
					...
					else
					{
						...
						checkbox();
						grid_success_box();	
						$('.grid_settings .error-message .alerts').text('Updated successfully!');	
					}
				}
			});
			break;
		...
		{
			grid_success_box();	
			$('.grid_settings .error-message .alerts').text('View Applied successfully!');		
			...
			api.column(0, {page:'current'} ).data().each( function ( group, i ) {     
				if ( last !== group ) {
						...
						else
						{
							...
							var back_color = 'style="background-color:#C0C0C0 !important"';
						}
						$(rows).eq(i).before(
							'<tr class="group total pointer_none" '+back_color+'><td><div class="total-sum"><b>Total Sum:</b></div><span class="text-clr">'+total_text+'</span></td></tr>'
						);
						last = group;
				}
