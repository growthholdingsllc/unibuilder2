...
$('#add_question').modal('hide');
	...
	$('#edit_save_question').hide();
		...
		$(document).on('click', '.add_question', function(){
			//alert(1);
			$('#survey_template_id').val($('#ub_survey_template_id').val());
		...
		$(document).on('click', '.edit_survey_question', function(){			
			...
			// alert(question_id);
			var template_id             =$('#ub_survey_template_id').val();
			...
			$('#survey_template_id').val(template_id);
			//alert($('#inc_app').val());
			if($('#inc_app').val() == 'Yes')
			{
			...
			$('#ub_survey_template_question_id').val(question_id);
			$('.selectpicker').selectpicker('refresh');
		});
...
function update_question_form(){
		...
		excluded: [':disabled'],
		 button: {
            selector: '#save_question , #edit_save_question',          
        },
        fields: {
            'question_notes': {
                validators: {
...
function save_question(ub_survey_template_question_id){
			// alert(ub_survey_template_question_id);
			var q_notes 		= $('#question_notes').val();
			var res_col_name 	= $('#res_col_name').val();
			var ans_type		= $('#ans_type').val();
			var inlcude_app		= $('#inc_app').val();						;
			$('#survey_field_list').append('<li class="panel panel-info"><div class="panel-heading"><table width="100%" class="table borderless"><tr><td width="50" rowspan="3"><img border="0" alt="sort" class="sort_img" src="' + imgLink + 'sort.png"></td><td width="150"><label>Answer Type:</label></td><td width="150"><span class="ans_type_field">'+ans_type+'</span></td><td width="120"><label>Response Feild:</label></td><td width="300"><span class="res_col_name_field">'+res_col_name+'</span></td><td>&nbsp;</td></tr><tr><td colspan="4" class="question_box"><p><label>Question:</label> <span class="q_notes_field">'+q_notes+'</span><span class="question_id hide">'+ub_survey_template_question_id+'</span></p></td><td><div class="pull-right"><button type="button" class="btn btn-blue edit_survey_question" id='+ub_survey_template_question_id+' value='+ub_survey_template_question_id+'><img border="0" alt="edit" class="uni_edit" src="'+imgLink + 'strip.gif"> Edit</button> <button type="button" class="btn btn-blue" id="'+ub_survey_template_question_id+'" onclick="deletetemplatequestion('+ub_survey_template_question_id+')"><img border="0" alt="edit" class="uni_delete" src="'+imgLink+'strip.gif"> Delete</button></div></td></tr><tr><td width="150"><label>Include Not applicable:</label></td><td colspan="4"><span class="inlcude_app_field">'+inlcude_app+'</span></td></tr></table></div></li>');
			
		} 
			
function edit_save_question(ub_survey_template_question_id){			
	var q_notes 		= $('#question_notes').val();
	var res_col_name 	= $('#res_col_name').val();
	var ans_type		= $('#ans_type').val();
	var inlcude_app		= $('#inc_app').val();	
...
//Add and stay
$('#survey_save_stay').on('click',function(e) {
...
//Add and back
$('#survey_save_back').on('click',function(e) {	
...
{
	// '.ans_type_field,.res_col_name_field,.q_notes_field,.inlcude_app_field'
	var question_id = $('.question_id').map(function() { return $(this).text(); }).get().join();
	
	var ub_survey_template_id = $('#ub_survey_template_id').val();
	var encoded_string = Base64.encode('survey/new_template/');
			...
			{	
				if($("#save_type").val() == 'save_and_back')
					...
					else if($("#save_type").val() == 'save_and_stay')
					{
						...
						var encoded_string_edit_log = Base64.encode('survey/new_template/' + response.insert_id);
...
$('#edit_save_question').click(function(e) {
 $("#save_question_type").val('edit');		
	...
	/* else{						
		edit_save_question()
		edit_new_template_question();	
	} */
});
/* function edit_new_template_question()
{
	var encoded_string = Base64.encode('survey/new_template_question/');
	var encoded_val = encoded_string.strtr(encode_chars_obj);
	var ub_survey_template_question_id = $('#ub_survey_template_question_id').val();
	var encoded_home_string = Base64.encode('survey/index/');
	var encoded_home_val = encoded_home_string.strtr(encode_chars_obj);	
...
function save_new_template_question()
{
	...
	$.ajax({
		...
		success: function(response) {	
			...
			{	
				...
				{
					save_question(response.insert_id);
				}
		        else
				{
					// alert("hello");
					edit_save_question(response.insert_id);
				}
				success_box();
				$(".alerts").html(response.message);	
				var encoded_string_edit_log = Base64.encode('survey/new_template/'+ survey_template_id);
				var encoded_edit_val = encoded_string_edit_log.strtr(encode_chars_obj);
				//save_question(response.insert_id);
				//console.log(encoded_edit_val);
				// window.location.href = encoded_edit_val;
...
function deletetemplate(ub_survey_template_id){
//alert(ub_survey_template_id);
    if(ub_survey_template_id > 0)
    {
    var encoded_delete_roles = Base64.encode('survey/delete_template/');
	 ...
	 $('#delete_confirm').click(function(){
		...
		if(conf == true){
			...
			$.ajax({
					...
					dataType: 'json',
					data: {'ub_survey_template_id':{ub_survey_template_id:ub_survey_template_id}},
					success: function(response) {   
						if(response.status == true)
						{   
							$(".error-message .alerts").removeClass('alert-danger');
    ...
    {               
        ...
        $(".error-message .alerts").removeClass('alert-success');
        $(".error-message").show();
        $(".alerts").html("Template id is not set");      
    }
}

function deletetemplatequestion(ub_survey_template_question_id){
    if(ub_survey_template_question_id > 0)
    {
    var encoded_delete_roles = Base64.encode('survey/delete_template_question/');
    var encoded_delete_val = encoded_delete_roles.strtr(encode_chars_obj);
    var index_string = Base64.encode('survey/index/');
    var index_url = index_string.strtr(encode_chars_obj);
	 var conf = $('#confirmModal').modal('show');
	 $('#delete_confirm').click(function(){
		 ...
		 //alert(conf);
		if(conf == true){
			//alert(ub_survey_template_question_id);return false;
			$('#confirmModal').modal('hide');
			$.ajax({
					type:'POST',
					url: base_url + encoded_delete_val,
					dataType: 'json',
					data: {'ub_survey_template_question_id':{ub_survey_template_question_id:ub_survey_template_question_id}},
					success: function(response) {   
						if(response.status == true)
						{   
							$(".error-message .alerts").removeClass('alert-danger');
		...
		else
		{
 		...
 		/*$('#cancel_conform').click(function(){

 				ub_survey_template_question_id = 0;
