$(function(){	
	default_image = base_url + 'assets/images/default.jpg'; 
	default_image1 = base_url + 'assets/images/drag_drop_signature.jpg'; 
	$(document).on('click','#profile_pic_delete', function(){
		if(typeof signature_img == 'signature_img'){			
			$('#profile-avatar').attr('src', ''+default_image1+'');
		}
		else{
			$('#profile-avatar').attr('src', ''+default_image+'');
		}
	});
});
  ...
  var Avatar = {
    ...
    handleDrop: function(files) {
      ...
      if (typeof file !== 'undefined' && file.type.match('image.*')) {
		...
		$('.alert_modal_txt').text("That file wasn't an image.");
    ...
    resizeImage: function(file, size, callback) {
      ...
      fileTracker.onabort = function() {
		...
		$('#alertModal').modal('show');
		$('.alert_modal_txt').text("The upload was aborted.");
      }
      fileTracker.onerror = function() {
        // alert("An error occured while reading the file.");
		$('#alertModal').modal('show');
		$('.alert_modal_txt').text("An error occured while reading the file.");
      }
