...
function pointer_none(){
	if($('.tab-con li').hasClass("active")){
			// $('.tab-con li.active').addClass('pointer_none');			
	}
}
$(function() {
	setTimeout(pointer_none, 1*500);
	//$('.tab-con li.active').addClass('pointer_none');
	...
	var hash = url.substring(url.indexOf("#"));
		...
		//gantt_chart();
		$(document).on('click','a[href = "#Gantt"]', function(e){	
	      gantt_chart();
        });
    });
	
/* 	$('#publish_confirm').click(function(){
		var table = $('#schedule_list').DataTable();
		...
		for(n = 0; n < length; n++)
		{
			if(n!=length-1)
			{
				scheduleids = scheduleids + data[n].ub_schedule_id + ',';
			}
			else
			{
				scheduleids = scheduleids + data[n].ub_schedule_id;
		...
		if(scheduleids.replace(/^\s+|\s+$/g,"").length != 0)
		{
			publish_schedules(scheduleids);
		}
		$('#confirmModal').modal('hide');
	});
 */	
function publish_schedules(action)
{
	if(action == 'publish_all')
	{
		$('#confirmModal h4').html('Are you sure? You need to publish all schedule(s).');
		var conf = $('#confirmModal').modal('show');
		$('#publish_confirm').click(function(){
					var conf = true;
					if(conf == true){
						$('#confirmModal').modal('hide');
		/* if(false == confirm('Are you sure? You need to publish all schedule(s).'))
		{
		...
		var scheduleids = '';
		var n = 0;
		var check_single_id = 0;
		if(length>0)
		{
			for(n = 0; n < length; n++)
			{
				if(data[n].publish_status == 'No')
				{
					check_single_id = check_single_id + 1;
					if(check_single_id == 1)
					{
						scheduleids = scheduleids + data[n].ub_schedule_id;
			...
			if(scheduleids.replace(/^\s+|\s+$/g,"").length != 0)
			{
				var encoded_url = Base64.encode('schedules/publish_schedules/');
				var ajax_encoded_url = encoded_url.strtr(encode_chars_obj);
				var post_data = 'scheduleids='+scheduleids;	
			...
			{
				// alert('Schedule list selected are already in published status!');
				$('#alertModal').modal('show');
				$('.alert_modal_txt').text('Schedule list selected are already in published status!');
				return false;
			}
		...
		{
			$('#alertModal').modal('show');
			$('.alert_modal_txt').text('Schedule list is empty!');
			return false;
		}
		}
		});
	...
	$.ajax({
		...
		success: function(response) {
			...
			gantt.config.columns=[
				...
				{name:"actual_start_date", label:"Start Date", align: "center", width:80 },
				{name:"actual_end_date", label:"End Date", align: "center", width:80 },
				{name:"duration_in_days",   label:"Duration",   align: "center", width:80 },
				{name:"assignedto",   label:"Assigned to",   align: "center", width:100 }
			];
			gantt.templates.tooltip_text = function(start,end,task,duration,assignedto){
				return "<b>Task:</b> "+task.text+"<br/><b>Start date:</b> " + 
				task.actual_start_date+ 
				"<br/><b>End date:</b> "+task.actual_end_date+ 
				"<br/><b>Duration:</b> "+task.duration_in_days+"<br/><b>Assigned to:</b> "+task.assignedto;
			};
			
			gantt.init("gantt_here");
			
			gantt.parse(tasks);				
			/* var gantt_w 	= $('.gantt_container').width();			
			var gantt_gr 	= $('.gantt_grid').width();			
			var gantt_ta 	= $('.gantt_task').width();	
			var overall_w   = gantt_w+gantt_gr+gantt_ta;			
			$('.gantt_hor_scroll').show();
			$('.gantt_hor_scroll').width(gantt_w).height(18);
			$('.gantt_hor_scroll').append('<div style="width: '+overall_w+'px;"></div>') *//* Gantt Scroll Issue Fixed 03-Nov-2015 */
		}
	});	
}	
function schedule_listview(calltype) {
		...
		// return false;
		// Ajax URL
		var encoded_url = Base64.encode('schedules/get_schedules/');
		var ajax_encoded_url = encoded_url.strtr(encode_chars_obj);
		// Data table Object
		...
		var dbobject = {					
							...
							'post_data':'{"assignedto":"'+assignedto+'","schdule_status":"'+  schdule_status+'", "tags":"'+tags+'","phase":"'+phase+'", "daterange":"'+daterange+'","event":"'+event+'","fetch_type":"'+fetch_type+'"}',
							'delete_data':'', 
...
//alert(dbobject.display_columns.toSource());
	...
	function baseline_listview() {
		...
		var dbobject = {
							...
							'delete_data':{},  
							'edit_data':{'index':0, 'url':'schedules/save_schedule/'},
							'display_columns' : [{"data": "title","bSortable": false},{"data": "is_completed","bSortable": false},{"data": "no_of_days","bSortable": false},
							{"data": "actual_start_date","bSortable": false},{"data": "actual_end_date","bSortable": false},{"data": "direct_shifts","bSortable": false},{"data": "duration_change","bSortable": false},{"data": "slip_days","bSortable": false},{"data": "assignedto","bSortable": false}],
...
$('#import_template').on('click', function(e){
	var values = $('#template_id').val();
	if(values == null || values == '' ){
		// alert('Please select cost code');
		$('#alertModal').modal('show');
		$('.alert_modal_txt').text('Please select a template');
		return false;
	}
	else
	{
