...
$("body").on("keyup", "#budget_amount, #quantity, #unit_cost", function(event){ 
...
$(document).on("click", "#insert_costcode", function(e){
	...
	var encoded_url = Base64.encode('template/budget/update_costcode/');
	...
	if(cost_variance_code!=''){
		if(check_costcode_array.indexOf(cost_variance_code) > 0) {
			alert("This cost code already exists"); return false;
		}
		xhr = $.ajax({
			type: "POST",
			dataType: "json",
...
function edit_estimate(ub_estimate_id)
{
	...
	else
	{
		...
		var encoded_url = Base64.encode('template/budget/get_estimate/');
		var ajax_encoded_url = encoded_url.strtr(encode_chars_obj);
...
function add_estimate_form() {
	// alert('add Estimate');
	var encoded_string = Base64.encode('template/budget/save_estimate/');
	...
	var ajaxData  = $("#save_estimate").serialize();	
		...
		success: function(response) {
			...
			if(response.status == true)
			{
                ...
                {
                    var encoded_string_edit_log = Base64.encode( 'template/budget/project_budget/');
...
function del_estimate_form() {
	// alert('coming to del');
	var encoded_string = Base64.encode('template/budget/del_estimate/');
	var encoded_val = encoded_string.strtr(encode_chars_obj);
	var success_msg = 'Successful';
	var failure_msg = 'Failed';
	var ajaxData  = $("#save_estimate").serialize();	
