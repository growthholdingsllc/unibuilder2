$(function(){
  ...
  if(request_count == 0)
  {
    $('#bid_save_release').hide();
    $('#flat-fee').attr('disabled',false);
    $('#line-items').attr('disabled',false);
  }
  else
  {
    $('#bid_save_release').show();
  }
});
$(function(){
...
$('select#schedule_id,select#before_or_after,#number_days').on('change',function() {
    ...
    if(schedule_id != ''){
    var encoded_string = Base64.encode('bids/get_schedule_date/');
...
$(function() {
	...
	{
		check_project_status('bids/index/');
	}
...
$(function() {  
    ...
    $('#bid_cancel').click(function(e) {   
         $('#current_tab').val('');
         var encoded_home_string = Base64.encode('bids/index/');
...
{
  var ub_bid_id = $('#ub_bid_id').val();
  var encoded_string = Base64.encode('bids/close_bid/');
  var encoded_val = encoded_string.strtr(encode_chars_obj);

  var encoded_home_string = Base64.encode('bids/save_bid/'+ub_bid_id);
  var encoded_home_val = encoded_home_string.strtr(encode_chars_obj);
...
function add_formval(){  
    var bidform = $('#bid_save').formValidation({
     ...
     button: {
            selector: '#bid_save_stay, #bid_save_new, #bid_save_back, #bid_save_release, #bidtab a'          
        },
        fields: {
            'package_title': {
                validators: {
                    notEmpty: {
                        message: 'The Package Title is required and cannot be empty'
                    },
...
function add_bid_form() {
    ...
    // Encode the String
    var encoded_string = Base64.encode('bids/save_bid/');
    var encoded_val = encoded_string.strtr(encode_chars_obj);
    ...
    if((status == 'In Progress' || ub_bid_id == 0 || $("#save_type").val() == 'save_and_release'))
    {
        $.ajax({
        url: base_url + encoded_val,
        dataType: "json",
        ...
        success: function(response) {  
            ...
            {  
                ...
                else if($("#save_type").val() == 'save_and_release')
                {
                  ...
                  var encoded_string_edit_log = Base64.encode( 'bids/save_bid/' + ub_bid_id);
                ...
                else if($("#save_type").val() == 'save_and_stay_tab')
                {
                     ...
                     var encoded_string_edit_log = Base64.encode( 'bids/save_bid/' + response.insert_id);
                     var encoded_edit_val = encoded_string_edit_log.strtr(encode_chars_obj);
...
function accept_bid(id)
{
    ...
    // Encode the String
    var ub_bid_id_request_id = id;
    var bid_id = $('#ub_bid_id').val();
    var encoded_string = Base64.encode('bids/accept_bid/'+ub_bid_id_request_id+'/'+bid_id);
    var encoded_val = encoded_string.strtr(encode_chars_obj);
    window.location.href = encoded_val;
...
function delete_bid(bid_ids_obj){
    if(bid_ids_obj > 0)
    {
    var encoded_delete_roles = Base64.encode('bids/delete_bid/');
  ...
  $('#confirmModal').modal('hide');
    $.ajax({
            ...
            dataType: 'json',
            data: {'ub_bid_id':{ub_bid_id:bid_ids_obj}},
            success: function(response) {   
                if(response.status == true)
                {   
                    $(".error-message .alerts").removeClass('alert-danger');
...
function delete_request(id)
{
    ...
    var id = id;
    var fields = id.split(/,/);
    var ub_bid_request_id = fields[0];
    var sub_contractor_id = fields[1];

    if(ub_bid_request_id > 0)
    {
    var encoded_delete_roles = Base64.encode('bids/delete_bid_request/');
    var encoded_delete_val = encoded_delete_roles.strtr(encode_chars_obj);
...
$('#delete_comment_confirm').click(function(){
    ...
    $.ajax({
            ...
            url: base_url + encoded_delete_val,
            dataType: 'json',
            data: {'ub_bid_request_id':{ub_bid_request_id:ub_bid_request_id}},
            success: function(response) {   
                if(response.status == true)
                {   
                    $("#request_list").load(location.href + " #request_list");
                    $('.selectpicker').selectpicker('refresh');
                    $("#sub_contractor_id option[value='" + sub_contractor_id + "']").prop("selected", false);					
                    $('.selectpicker').data('selectpicker', null);
					          $('.bootstrap-select').remove();
					          $('.selectpicker').selectpicker();
    ...
    {               
        ...
        $(".error-message .alerts").removeClass('alert-success');
        $(".error-message").show();
        $(".alerts").html("Bid Request id is not set");      
    }
...
function add_po(val)
{
    ...
    var id = $('#ub_bid_id').val();
    var sub_id = $('#sub_id').val();
    var project_id = $('#project_id').val();
    var user_id = val;
    var encoded_string = Base64.encode('budget/save_po_co/PO/0/'+user_id+'/'+user_id+'/'+sub_id+'/'+project_id);
    var encoded_val = encoded_string.strtr(encode_chars_obj);
    window.location.href = encoded_val;
