    ...
    <!-- The file upload form used as target for the file upload widget -->   
        ...
        <div class="row fileupload-buttonbar">
            <div class="col-xs-12">
				...
				<div class="col-xs-12">
                    <p>Allowed Extensions (xls, xlsx, csv, audio, wav, common, doc, one, docx, docm, txt, odt, folder, tif, gif, png, jpg, jpeg, tiff, pdf, avi, mp4, mov, xps, 3gp, 3gpp, dat, zip, rar)</p>
				...
				<div class="file-button col-xs-12 m-top">
					...
					<span class="pull-left sel-all">Select All <input type="checkbox" class="toggle"></span>					
		...
		<input type="hidden" name="folder_id" class="folder_id" value="<?php echo isset($choice_folder_id)?$choice_folder_id:'' ?>" />
        <input type="hidden" name="temp_directory_id" class="temp_directory_id" value="<?php echo isset($choice_dir_id)?$choice_dir_id:'' ?>" /> 
        <input type="hidden" name="temp_file_path" id="temp_file_path" value="" /> 
...
<ul class="upload_div">
    <li class="template-download ">
        ...
        <div class="col-xs-6">
            <p class="name">
                {% if (file.url) { %}
                    <a href="{%=file.url%}" class="doc_name" title="{%=file.name%}" download="{%=file.name%}" target="_blank" {%=file.thumbnailUrl?'data-gallery':''%}>{%=file.name%}</a>
                {% } else { %}
                    <span>{%=file.name%}</span>
                {% } %}
            </p>
        ...
        <div class="col-xs-2">
            {% if (file.deleteUrl) { %}
                <button class="btn btn-danger delete delete_btn pull-right" data-type="{%=file.deleteType%}" data-url="{%=file.deleteUrl%}"{% if (file.deleteWithCredentials) { %} data-xhr-fields='{"withCredentials":true}'{% } %}>
                    <i class="glyphicon glyphicon-trash"></i>
                    <span>Delete</span>
                </button>                
            {% } else { %}
