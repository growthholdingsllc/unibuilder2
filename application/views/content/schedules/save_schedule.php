...
<form id="save_schedule" class="form-horizontal" method="post" name="save_schedule">
         ...
         <div class="top-search pull-right">
            <div class="pull-right">
               ...
               <button type="button" class="btn btn-gray  pull-right m-left-1" id="cancel" name="cancel"> <img src="<?php echo IMAGESRC.'strip.gif'; ?>" class="uni_cancel_new"/> Cancel</button>
				...
				<?php
				if(isset($this->user_role_access[strtolower('schedules')][strtolower('delete')]) && $this->user_role_access[strtolower('schedules')][strtolower('delete')] == 1)
				{ 
					if(isset($schedule_details['ub_schedule_id']))
					{
						if(isset($this->project_status_check) && $this->project_status_check == 1)
						{
				?>
					<button class="btn btn-blue pull-right m-left-1" type="button" id="<?php if(isset($schedule_details['ub_schedule_id'])) echo $schedule_details['ub_schedule_id']; ?>" onclick="delete_schedule(this.id)">
   ...
   <div class="row m-top five-col">
      <div class="col-xs-3 <?php if($this->project_id == '' || $this->project_id == 0){ echo 'no_project_selected'; } ?>">
         <?php 
            if( ! empty($this->project_id))
            {
            //echo '<label>Project Name</label> : '.$this->project_name;			  
            echo '<input type="hidden" name="project_id" id="project_id" value="'.$this->project_id.'" readonly />';
            }
            ?>
      </div>
   ...
   <div class="row m-top five-col">
      ...
      <div class="col-xs-3">
         ...
         <?php 
            $reminder_selected = '';
            if(isset($schedule_details['reminder_id']) && $schedule_details['reminder_id'] != '')
            {
            	$reminder_selected = $schedule_details['reminder_id'];
            }
            echo form_dropdown('reminder',$schedule_reminder_dropdown, $reminder_selected, "class='selectpicker form-control' id='reminder' data-live-search='true'"); 
             ?>
			 <input type="hidden" name="hide_reminder_id" id="hide_reminder_id" value="<?php echo (isset($reminder_selected) && $reminder_selected>0)?$reminder_selected:0;?>">
      </div>
      <div class="col-xs-3">
         <label>Tags</label>
         <div class="col-xs">
	  ...
	  <?php
		if(isset($schedule_details['ub_schedule_id']) && $schedule_details['ub_schedule_id']>0)
		{
	  ?>
      <div class="col-xs-3">
         <label>Start Creating Baseline</label>
         <p>
         ...
         <div class="tab-con pull-left">
            <div class="tabpanel">
               ...
               <ul class="nav nav-tabs" role="tablist">
				  ...
				  ?>
                  <li role="notes" class="<?php if(isset($this->user_account_type) && BUILDERADMIN != $this->user_account_type){ echo 'active'; } ?>"> <a href="#notes" data-toggle="tab">Notes</a> </li>
				  <?php 
				  if(isset($this->user_account_type) && BUILDERADMIN == $this->user_account_type)
				  {
               ...
               <div class="tab-content">
                  ...
                  <div class="tab-pane" id="view_access">
                     <?php $this->load->view('content/schedules/viewaccess_tab_content'); ?>
                  </div>
				  <?php 
				  }
				  ?>
                  <div class="tab-pane <?php if(isset($this->user_account_type) && BUILDERADMIN != $this->user_account_type){ echo 'active'; } ?>" id="notes">
                  ...
                  <div class="tab-pane" id="files">
                     <?php $this->load->view('content/schedules/files_tab_content'); ?>
                  </div>
               </div>
            </div>
         </div>
   ...
   <input type="hidden" id="schedule_id" name="schedule_id" value="<?php echo isset($schedule_details['ub_schedule_id'])?$schedule_details['ub_schedule_id']:'';?>"/>
   <input type="hidden" id="project_id" name="project_id" value="<?php echo isset($this->project_id)?$this->project_id:0;?>"/>
   ...
   $hide_db_duration ='';
   // Base start date
   if(isset($schedule_details['base_start_date']) && $schedule_details['base_start_date'] != '')
   {
		$hide_db_start_date = $schedule_details['base_start_date'];
   }
   else if(isset($schedule_details['start_date']) && $schedule_details['start_date'] != '')
   {
		$hide_db_start_date = $schedule_details['start_date'];
   }
    // Base end date
   if(isset($schedule_details['base_end_date']) && $schedule_details['base_end_date'] != '')
   {
		$hide_db_end_date = $schedule_details['base_end_date'];
   }
   else if(isset($schedule_details['end_date']) && $schedule_details['end_date'] != '')
   {
		$hide_db_end_date = $schedule_details['end_date'];
   }
   // Base duration
   if(isset($schedule_details['base_no_of_days']) && $schedule_details['base_no_of_days'] != '')
   {
		$hide_db_duration = $schedule_details['base_no_of_days'];
   }
   else if(isset($schedule_details['no_of_days']) && $schedule_details['no_of_days'] != '')
   {
		$hide_db_duration = $schedule_details['no_of_days'];
   }
   
   ?>
   
   
   <input type="hidden" id="hide_db_start_date" name="hide_db_start_date" value="<?php echo $hide_db_start_date;?>"/>
   <input type="hidden" id="hide_db_end_date" name="hide_db_end_date" value="<?php echo  $hide_db_end_date;?>"/>
   <input type="hidden" id="hide_db_duration" name="hide_db_duration" value="<?php echo $hide_db_duration;?>"/>
   <input type="hidden" id="db_start_date" name="db_start_date" value="<?php echo isset($schedule_details['start_date'])?$schedule_details['start_date']:''; ?>"/>
   <input type="hidden" id="db_end_date" name="db_end_date" value="<?php echo isset($schedule_details['end_date'])?$schedule_details['end_date']:''; ?>"/>
   <input type="hidden" id="db_duration" name="db_duration" value="<?php echo isset($schedule_details['no_of_days'])?$schedule_details['no_of_days']:''; ?>"/>
...
<script type="text/javascript">  
   ...
   var schedule_id = '<?php echo isset($schedule_details['ub_schedule_id'])?$schedule_details['ub_schedule_id']:0;?>';
...
<script type="text/javascript" src="<?php echo JSSRC.'save_schedule.js';?>"></script>