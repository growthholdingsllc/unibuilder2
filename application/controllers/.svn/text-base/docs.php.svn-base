...
class Docs extends UNI_Controller {
	/** 
	 * Docs Class
	 * 
	 * @package: Docs
	...
	*/
	public function index($folder_id = 0, $project_id = 0)
	{
		...
		$docs_folder_id = $folder_id;
		/* Below code was modified by chandru 08-10-2015 */
		$get_folder_id = array(
		...
		if ($all_folder['aaData']['0']['ub_doc_folder_id'] === $docs_folder_id)
		{
			$docs_folder_id = 0;
		}
		$data = array(
		...
		'title'        => "DOCS",		
		'content'      => 'content/docs/docs',
        'page_id'      => 'docs'     
		);


		// Block for breadcrum in Docs.
		...
		if (isset($all_folder['aaData']['0']['ub_doc_folder_id']) && !empty($all_folder['aaData']['0']['ub_doc_folder_id'])) 
		{
			...
			$response_status= $this->uni_set_session('BREADCRUMB', $breadcrum_array);
		}
		$ub_breadcrum_array = array_filter($this->uni_session_get('BREADCRUMB'));

		if (count($ub_breadcrum_array) <= 1) 
		{
			$breadcrum_data_array = array_filter($this->uni_session_get('BREADCRUMB'));
			$_SESSION["docs_breadcrumb"] = $breadcrum_data_array;
		}
		else
		{
			$session_breadcrum_array = $_SESSION["docs_breadcrumb"];
		...
		$_SESSION["docs_breadcrumb"] = $breadcrum_data;

		//echo "<pre>"; print_r($ub_breadcrum_array); print_r($_SESSION["breadcrum1"]); 

		foreach ($breadcrum_data as $key => $value) 
			{
				$val = explode(',', $value);
				$breadcrum_link = $this->crypt->encrypt('docs/index/'.$key.'/'.$val[1]);
	...
	public function get_folder_details()
	{
		if(!empty($this->input->post()))
		{
			...
			{ 
				...
				{
					$ext = pathinfo($result_array[$i]['folder_name'], PATHINFO_EXTENSION);
	...
	public function create_new_dir()
	{
		if(!empty($this->input->post()))
		{
			...
			if(TRUE === $result['status'])
			{
				...
				//echo "<pre>";print_r($builder_data);exit;
		...
		if(!empty($this->input->post()))
		{
			...
			if(TRUE === $result['status'])
			{
				...
				else
				{
					...
					//echo "<pre>";print_r($owner_access_folder['aaData']);exit();
				}
			}
		}
	...
	*/   
	public function get_folder_path($folderid = 0)
	{
		...
		{
			echo "Sorry please select some other folder.";
		}
	}
	/**
	*
