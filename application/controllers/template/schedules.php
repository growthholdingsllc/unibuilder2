...
class Schedules extends UNI_Controller {
	...
	public function __construct()
    {
		...
		$this->load->model(array('Mod_schedule','Mod_project','Mod_general_value','Mod_saved_search','Mod_user','Mod_notification','Mod_doc','Mod_template','Mod_template_schedule'));
    }
	/** 
	* Get all schedule
	* 
	...
	*/
	public function index()
	{
		...
		'content'      			=> 'template/content/schedules/schedules',
        'page_id'      			=> 'schedules',
	  ...
	  if(!empty($this->input->post()))
		{
			...
			if(TRUE === $result['status'])
			{
				...
				{
					$post_array[] = array(
									'field_name' => 'SCHEDULE.template_id',
				...
				if(isset($result['data']['event']) && $result['data']['event']!='' && $result['data']['event'] != 'null')
				 ...
				 $where_str = $this->Mod_template_schedule->build_where($post_array);	
                ...
                $schedule_args = array('select_fields' => array('SCHEDULE.ub_template_schedule_id', 'SCHEDULE.title', 
				...
				$result_data = $this->Mod_template_schedule->get_schedules($schedule_args);
				...
				else
				{
				 $count = count($result_data['aaData']);
					 ...
					 for($i=0;$i<$count;$i++)
					{ 
						// echo '<pre>';print_r($result_data['aaData'][$i]['assigned_users']);exit;
						 if(isset($result_data['aaData'][$i]['assigned_users'])){
							...
							$assign_post_array[] = array(
							 ...
							 $where = $this->Mod_template_schedule->build_where($assign_post_array);
							 $assign_post_array = '' ;
							 $query_array = array('select_fields' => array('GROUP_CONCAT(CONCAT_WS(" ",USER.first_name,USER.last_name )SEPARATOR ",<br>") AS assignees'),
							 'where_clause' => $where,
							 );			
							$resultdata[] = $this->Mod_template_schedule->get_users($query_array);
					...
					$total_count_array = $this->Mod_template_schedule->get_schedules(array(
												'select_fields' => array('COUNT(SCHEDULE.ub_template_schedule_id) AS total_count'),
												'where_clause' => $where_str,
												));
				...
				$this->Mod_template_schedule->response($result_data);
			}
			else
			{
				$this->Mod_template_schedule->response($result);
			}
		}
		else
		{
	...
	*/	
	public function save_schedule($schedule_id = 0)
	{
		...
		$data = array(
		'title'        			=> "Save Schedule",		
		'content'      			=> 'template/content/schedules/save_schedule',
        'page_id'      			=> 'schedules',
		'data_table'   			=>'data_table',			
		'date_all' 	   			=>'date_all',						
		'predecessor_list' 	   	=>'predecessor_list',
		...
		if($this->schedule_id>0)
		{
			...
			// echo '<pre>';print_r($get_schedule_details);exit;
			// Look for successful retrieval 
			if(TRUE === $get_schedule_details['status'])
			{
				if(TRUE === $get_schedule_details['status'])
				{
					$project_info = $this->Mod_template->get_template(array(
							'select_fields' =>array('template_name'), 
							'where_clause' => array('ub_template_id'=>$get_schedule_details['schedule_details']['template_id'])
					));
					// echo '<pre>';print_r($project_info);exit;
					if(TRUE === $project_info['status'])
					{
						$this->template_id = $get_schedule_details['schedule_details']['template_id'];
						$this->template_name = $project_info['aaData'][0]['template_name'];
					}
					else
					{
		...
		$get_schedule_predecessor = array('builder_id' => $this->builder_id,'template_id'=>$this->template_id,'template_schedule_id' =>($this->schedule_id>0)?$this->schedule_id:0);
		if($get_schedule_predecessor['template_schedule_id'] < 1)
		{
			$where_str = "TEMPLATE_SCHEDULE.builder_id = ".$this->builder_id." AND TEMPLATE_SCHEDULE.template_id = ".$this->template_id." AND TEMPLATE_SCHEDULE.is_delete = 'No'";
		}
		else
		{
			$where_str = "TEMPLATE_SCHEDULE.builder_id = ".$this->builder_id." AND TEMPLATE_SCHEDULE.template_id = ".$this->template_id." AND TEMPLATE_SCHEDULE.is_delete = 'No' AND TEMPLATE_SCHEDULE.ub_template_schedule_id <> ".$get_schedule_predecessor['template_schedule_id'];
		}
		// echo '<pre>';print_r($where_str);exit;
		$schedule_predecessor_result = $this->Mod_template_schedule->get_all_predecessors(array(
					'select_fields' => array('TEMPLATE_SCHEDULE.ub_template_schedule_id', 'TEMPLATE_SCHEDULE.title', 'TEMPLATE_SCHEDULE.start_date','TEMPLATE_SCHEDULE.end_date'),
					'where_clause' => $where_str
					),$get_schedule_predecessor); 
		//echo '<pre>';print_r($schedule_predecessor_result);exit;			
		// $schedule_predecessor_result = $this->Mod_template_schedule->get_all_predecessors($get_schedule_predecessor);
		if(TRUE === $schedule_predecessor_result['status'])
		{
			$schedule_predecessor_dropdown = $this->Mod_schedule->build_ci_dropdown_array($schedule_predecessor_result['aaData'],'ub_template_schedule_id','title,start_date,end_date','append');
			$data['schedule_predecessor_dropdown'] = $schedule_predecessor_dropdown;
		}
		// echo '<pre>';print_r($schedule_predecessor_dropdown);exit;
		...
		// echo '<pre>';print_r($this->schedule_id);
		if($this->schedule_id > 0 )
		{
			...
			$where_str = "SCHEDULE_PREDECESSOR_INFO.template_schedule_id = ".$this->schedule_id." AND is_delete = 'No'";
			$assigned_predecessors_result = $this->Mod_template_schedule->get_schedule_predecessors(array(
					'select_fields' => array('SCHEDULE_PREDECESSOR_INFO.ub_template_schedule_predecessor_info_id', 'SCHEDULE_PREDECESSOR_INFO.template_schedule_id', 'SCHEDULE_PREDECESSOR_INFO.parent_id','SCHEDULE_PREDECESSOR_INFO.predecessor_type', 'SCHEDULE_PREDECESSOR_INFO.lag'),
					'where_clause' => $where_str
					)); 
			// $get_assigned_predecessors = array('schedule_id' => $this->schedule_id);
			// $assigned_predecessors_result = $this->Mod_template_schedule->get_schedule_predecessors($get_assigned_predecessors);
			// echo '<pre>';print_r($assigned_predecessors_result);exit;
			...
			if(isset($result['data']) && count($result['data'])>0 && $action_type != '')
			{
				...
				$db_start_date = $result['data']['hide_db_start_date'];
				$db_end_date = $result['data']['hide_db_end_date'];
				$db_duration = $result['data']['hide_db_duration'];
				$start_date_flag = 0;
				$end_date_flag = 0;	
				$duration_flag = 0 ;
				...
				// echo '<pre>';print_r($db_predecessor_count);
				// echo '<pre>';print_r($predecessor_count);exit;
				...
				$update_result = $this->Mod_template_schedule->update_schedule($result['data']);
		...
		// echo '<pre>';print_r('else');exit;
			...
			{
				$add_result = $this->Mod_template_schedule->add_schedule($result['data']);
	...
	*/	
	private function get_schedule_info($schedule_id=0)
	{
		...
		if($schedule_id>0)
		{
			...
			$where_str = "TEMPLATE_SCHEDULE.ub_template_schedule_id = ".$schedule_id." AND TEMPLATE_SCHEDULE.is_delete = 'No'";
			// echo '<pre>';print_r($where_str);exit;
			$get_schedule_details = $this->Mod_template_schedule->get_schedule_details(array(
					'select_fields' => array('TEMPLATE_SCHEDULE.ub_template_schedule_id', 'TEMPLATE_SCHEDULE.title', 'TEMPLATE_SCHEDULE.phase', 'TEMPLATE_SCHEDULE.colour', 'TEMPLATE_SCHEDULE.start_date', 'TEMPLATE_SCHEDULE.base_start_date', 'TEMPLATE_SCHEDULE.end_date', 'TEMPLATE_SCHEDULE.base_end_date', 'TEMPLATE_SCHEDULE.no_of_days', 'TEMPLATE_SCHEDULE.base_no_of_days', 'TEMPLATE_SCHEDULE.tags', 'TEMPLATE_SCHEDULE.is_completed', 'TEMPLATE_SCHEDULE.builder_notes', 'TEMPLATE_SCHEDULE.subcontractor_notes', 'TEMPLATE_SCHEDULE.owner_notes', 'TEMPLATE_SCHEDULE.all_notes', 'TEMPLATE_SCHEDULE.publish_status', 'TEMPLATE_SCHEDULE.template_id'),
					'where_clause' => $where_str
					)); 
					// echo '<pre>';print_r($get_schedule_details);exit;
			if(TRUE === $get_schedule_details['status'])
			{
				$data['status'] = $get_schedule_details['status'];
				$data['message'] = $get_schedule_details['message'];
				$data['schedule_details'] = $get_schedule_details['aaData'][0];
				$multiple_tag = $get_schedule_details['aaData'][0]['tags'];
				...
				$data['schedule_details']['start_date'] = date("m/d/Y", strtotime($data['schedule_details']['start_date']));
				$data['schedule_details']['end_date'] = date("m/d/Y", strtotime($data['schedule_details']['end_date']));
				$data['schedule_details']['base_start_date'] = date("m/d/Y", strtotime($data['schedule_details']['base_start_date']));
				$data['schedule_details']['base_end_date'] = date("m/d/Y", strtotime($data['schedule_details']['base_end_date']));
			}		
		}
		else
		{
	...
	public function get_schedules_gantt()
	{
		$gantt_param_array = array();
			 ...
			 if( ! empty($this->input->post()))
		   {
			...
			$gantt_param_array['dateformat']  = $this->user_session['date_format'];
	...
	*/	
	...
	public function get_schedules_phase_list()
	{
		...
		$datetime_array = array('SCHEDULE.start_date'=> 'start_date','SCHEDULE.end_date'=> 'end_date');
		 ...
		 'SCHEDULE.is_completed,'.$this->Mod_user->format_user_datetime($datetime_array)),
		 'join'=> array('user'=>'Yes','builder'=>'Yes'),
		 'where_clause' => $where_str,
		 'order_clause' => $order_by_where,
		 ...
		 foreach($result_data['aaData'] as $key => $val)
			{
				...
				if(!empty($this->project_id))
				{
				 ...
				 // echo $where_str;exit;
				 $datetime_array = array('SCHEDULE.start_date'=> 'start_date','SCHEDULE.end_date'=> 'end_date');
				 $query_array = array('select_fields' => array(('SCHEDULE.title AS schedule_item_name'),'SCHEDULE.no_of_days','SCHEDULE.start_date','SCHEDULE.end_date','SCHEDULE.is_completed', 'GROUP_CONCAT(CONCAT_WS(" ",USERS.first_name,USERS.last_name )SEPARATOR ",<br>") AS assigned_users,'.$this->Mod_user->format_user_datetime($datetime_array)),
			...
			if(TRUE === $result['status'])
			{
				if(isset($result['data']['schedule_id']) && $result['data']['schedule_id'] >0)
				{
					...
					if(TRUE === $schedule_shift_history_result['status'])
					{
						...
						{
							$shift_history_row['base_start_date'] = date('m/d/Y',strtotime($shift_history_row['base_start_date']));
							$shift_history_row['base_end_date'] = date('m/d/Y',strtotime($shift_history_row['base_end_date']));
